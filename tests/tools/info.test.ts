...
describe("info tools", () => {
  ...
  it("list_networks passes filters through to the client", async () => {
    const get = vi.fn<AnsetaApi["get"]>(async () => [{ network: "solana", type: "solana", testnet: false }]);
  ...
  it("returns a model-readable error instead of throwing", async () => {
    const get = vi.fn<AnsetaApi["get"]>(async () => { throw new AnsetaApiError(400, "BAD", "nope"); });
    const ctx = stubClient({ get });
  ...
  it("notes truncation when the list is capped", async () => {
    ...
    const get = vi.fn<AnsetaApi["get"]>(async () => rows);
    const ctx = stubClient({ get });
    const result = await toolNamed("list_tokens").handler({}, ctx);
